package backend.rideme.auth.storages;
...
import backend.rideme.auth.exceptions.StorageFileNotFoundException;
...
public class FileSystemStorageService implements StorageService {
    ...
    public Stream<Path> loadAll() {
        try {
            ...
            throw new StorageFileNotFoundException.StorageException("Failed to read stored files", e);
        }
    ...
    public void init() {
        try {
            ...
            throw new StorageFileNotFoundException.StorageException("Could not initialize storage", e);
        }
    }
	...
	public void store(MultipartFile file, String imageName) {
		...
		try {
            if (file.isEmpty()) {
                throw new StorageFileNotFoundException.StorageException("Failed to store empty file " + file.getOriginalFilename());
            }
            ...
            throw new StorageFileNotFoundException.StorageException("Failed to store file " + file.getOriginalFilename(), e);
        }
	}
    ...
    public void store(File file, String type, String filename){
        ...
        try {
            if (!file.exists()) {
                throw new StorageFileNotFoundException.StorageException("Failed to store empty file " + file.getName());
            }
            Path path = rootLocation.resolve(type).resolve(filename);
            if (Files.exists(path)) {
                Files.delete(path);
            ...
            throw new StorageFileNotFoundException.StorageException("Failed to store file " + file.getName(), e);
        }
    }
